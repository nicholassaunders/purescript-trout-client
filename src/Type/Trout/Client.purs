...
instance clientErrorJSONClientError :: ClientError JSONClientError where
  printError (RequestError error) = "Request error: " <> Affjax.printError error
  printError (DecodeError error) = "Decode error: " <> printJsonDecodeError error
...
instance hasMethodClientMethodJson
  ...
  getMethodClients method _ req = do
    toAffjaxRequest req
      ...
      # request
      # map (lmap RequestError >>> (rmap $ _.body >>> decodeJson >>> lmap DecodeError) >>> join)
